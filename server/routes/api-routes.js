...
router.get("/games/:gameRoom/scoreboard", async (req, res) => {
  ...
  Object.values(rounds).forEach((round, index) => {
    ...
    Object.keys(teamTotals).forEach(team => {
      teamTotals[team] = round.questions.reduce(
        (prev, curr) =>
          prev +
          curr.teams.filter(t => t.teamName === team && t.isCorrect).length,
        0
      );
...
router.delete("/games/:gameRoom/team/:teamName", async (req, res) => {
  ...
  if (req.session.gameRoomName === gameRoom && req.session.quizMaster) {
    ...
    if (req.session.gameRoomName !== gameRoom) {
      console.log(
        `Error: Team delete called for non matching game room. Session room: ${req.session.gameRoomName}  Game room: ${gameRoom}`
      );
    }
...
router.put("/games/:gameRoom/team/:teamName", async (req, res) => {
  ...
  if (req.session.gameRoomName === gameRoom && req.session.quizMaster) {
    ...
    if (req.session.gameRoomName !== gameRoom) {
      console.log(
        `Error: Team accept called for non matching game room. Session room: ${req.session.gameRoomName}  Game room: ${gameRoom}`
      );
    }
  }
});
...
*/
router.post(
  "/game/:gameRoom/ronde/:rondeID/question/:questionID/team/:teamName/answer",
  async (req, res) => {
    const gameRoomName = req.params.gameRoom;
    ...
    if (req.session.gameRoomName === gameRoomName) {
      ...
      currentGame.rondes[roundID].vragen[questionID].team_antwoorden.forEach(
        function(arrayItem, key) {
          if (
            arrayItem.team_naam.includes(teamName) &&
            arrayItem.team_naam === teamName
          ) {
      ...
      if (isAlreadyAnswered) {
        currentGame.rondes[roundID].vragen[questionID].team_antwoorden[
          teamKey
...
*/
router.get(
  "/game/:gameRoom/ronde/:rondeID/question/:questionID/answers",
  async (req, res) => {
    const gameRoom = req.params.gameRoom;
    const roundID = req.params.rondeID - 1;
    const questionID = req.params.questionID - 1;
    ...
    //Check of isset session gameRoomName & is quizMaster
    if (
      req.session.gameRoomName === gameRoom &&
      (req.session.quizMaster || req.session.scoreBoard)
    ) {
      let currentGame = await Games.findOne({ _id: gameRoom });

      if (
        (currentGame.game_status === "question_closed" &&
          req.session.scoreBoard) ||
        req.session.quizMaster
      ) {
        return res.json({
          success: true,
          answers:
  ...
  async (req, res) => {
    ...
    if (req.session.gameRoomName === gameRoomName && req.session.quizMaster) {
      ...
      if (isAnswered) {
        ...
        //       team.round_scores = team.round_scores || {};

        //       team.round_scores[roundID] = {
        //         score: ((team.round_scores[roundID] || {}).score || 0) + 1,
