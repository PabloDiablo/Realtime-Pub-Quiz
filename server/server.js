    ...
    req.session.reload(err => {
      ...
      if (data.messageType === "NEW TEAM") {
        for (var key in players) {
          if (players.hasOwnProperty(key)) {
            if (
              (players[key].quizMaster &&
                players[key].gameRoomName === gameRoom) ||
              (players[key].scoreBoard &&
                players[key].gameRoomName === gameRoom)
            ) {
              players[key].send(
                JSON.stringify({
        ...
        for (var key in players) {
          if (players.hasOwnProperty(key)) {
            if (
              !players[key].quizMaster &&
              players[key].gameRoomName === gameRoom &&
              players[key].teamName === data.teamName
            ) {
              players[key].send(
                JSON.stringify({
      ...
      if (data.messageType === "ASKING QUESTION") {
        ...
        for (var key in players) {
          if (players.hasOwnProperty(key)) {
            if (
              (players[key].quizMaster || players[key].scoreBoard) &&
              players[key].gameRoomName === gameRoom
            ) {
              players[key].send(
                JSON.stringify({
