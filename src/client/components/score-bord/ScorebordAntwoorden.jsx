...
class ScorebordAntwoordUI extends React.Component {
  getTeams() {
    ...
    return currentTeamsScoreboard.map(teamName => {
      let isAnswered = teamName.isAnswered ? (
        <b style={{ color: "#28a745" }}>Question answered</b>
      ) : (
        <b style={{ color: "#dc3545" }}>No answer given yet</b>
      );
      return (
        <Col md={{ span: 4 }} key={teamName._id}>
          <Card>
            <Card.Body>
              ...
              <Card.Text className={"text-center success"}>
                {isAnswered}
              </Card.Text>
            </Card.Body>
          </Card>
...
export const ScorebordAntwoorden = ReactRedux.connect(mapStateToProps)(
  ScorebordAntwoordUI
);
