...
class ScorebordBeoordelingUI extends React.Component {
  getTeams() {
    return this.props.teamAnswers.map(teamName => {
      let correctOrWrong = "";
      if (teamName.correct) {
        correctOrWrong = (
          ...
          <Card>
            <Card.Body>
              ...
              <Card.Text className={"text-center success"}>
                <strong>{teamName.gegeven_antwoord}</strong>
              </Card.Text>
              <Card.Text className={"text-center success"}>
                {correctOrWrong}
              </Card.Text>
...
export const ScorebordBeoordeling = ReactRedux.connect(mapStateToProps)(
  ScorebordBeoordelingUI
);
